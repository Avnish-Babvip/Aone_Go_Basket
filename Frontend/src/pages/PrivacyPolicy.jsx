...
import { getPrivacyPolicy } from "../features/actions/cms";

const PrivacyPolicy = () => {
  const dispatch = useDispatch();
  const { privacyData, cmsLoading } = useSelector((state) => state.cms);
  const { siteData } = useSelector((state) => state.home);

  const sections = Array.isArray(privacyData) ? privacyData : [];
      ...
      {cmsLoading ? (
        ...
        <>
          ...
          <div className="relative w-full h-16 sm:h-24 md:h-32 lg:h-50 xl:h-56 overflow-hidden flex flex-col items-center justify-center border-b border-gray-100">
            <img
              src={`${import.meta.env.VITE_REACT_APP_IMAGE_URL_2}/${siteData?.common?.banner?.[0]}`}
